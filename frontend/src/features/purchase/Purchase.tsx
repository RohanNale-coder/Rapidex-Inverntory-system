...
const Purchase = () => {
  ...
  const [editingItem, setEditingItem] = useState<PurchaseOrder | PurchaseInvoice | null>(null);
  const [formData, setFormData] = useState({
    orderNumber: '', supplierId: 0, warehouseId: 0, orderDate: '', 
    expectedDeliveryDate: '', status: 'pending', totalAmount: 0, notes: '',
    invoiceNumber: '', purchaseOrderId: 0, invoiceDate: '', dueDate: '', 
    amount: 0, paidAmount: 0
  });
  const queryClient = useQueryClient();

  const { data: orders = [], isLoading: ordersLoading } = useQuery('purchaseOrders', async () => {
    const response = await axios.get('/purchase/orders');
    return response.data;
  });

  const { data: invoices = [], isLoading: invoicesLoading } = useQuery('purchaseInvoices', async () => {
    const response = await axios.get('/purchase/invoices');
    return response.data;
  });
  ...
  const createMutation: any = useMutation(
    async (data: any) => {
      const response = tabValue === 0 ? await axios.post('/purchase/orders', data) : await axios.post('/purchase/invoices', data);
      return response.data;
    },
    {
      onSuccess: () => {
        queryClient.invalidateQueries(tabValue === 0 ? 'purchaseOrders' : 'purchaseInvoices');
        setOpen(false);
        resetForm();
  ...
  const updateMutation: any = useMutation(
    async ({ data, id }: { data: any; id: number }) => {
      const response = tabValue === 0 ? await axios.put(`/purchase/orders/${id}`, data) : await axios.put(`/purchase/invoices/${id}`, data);
      return response.data;
    },
    {
      onSuccess: () => {
  ...
  const deleteMutation: any = useMutation(
    async (id: number) => {
      const response = tabValue === 0 ? await axios.delete(`/purchase/orders/${id}`) : await axios.delete(`/purchase/invoices/${id}`);
      return response.data;
    },
    {
      onSuccess: () => {
  ...
  const handleEdit = (item: PurchaseOrder | PurchaseInvoice) => {
    setEditingItem(item);
    const isOrder = tabValue === 0;
    setFormData({
      orderNumber: isOrder ? (item as PurchaseOrder).orderNumber : '',
      supplierId: isOrder ? (item as PurchaseOrder).supplierId : 0,
      warehouseId: isOrder ? (item as PurchaseOrder).warehouseId : 0,
      orderDate: isOrder ? (item as PurchaseOrder).orderDate : '',
      expectedDeliveryDate: isOrder ? (item as PurchaseOrder).expectedDeliveryDate : '',
      status: item.status || 'pending',
      totalAmount: isOrder ? (item as PurchaseOrder).totalAmount : (item as PurchaseInvoice).amount || 0,
      notes: isOrder ? (item as PurchaseOrder).notes || '' : '',
      invoiceNumber: !isOrder ? (item as PurchaseInvoice).invoiceNumber : '',
      purchaseOrderId: !isOrder ? (item as PurchaseInvoice).purchaseOrderId : 0,
      invoiceDate: !isOrder ? (item as PurchaseInvoice).invoiceDate : '',
      dueDate: !isOrder ? (item as PurchaseInvoice).dueDate : '',
      amount: !isOrder ? (item as PurchaseInvoice).amount || 0 : 0,
      paidAmount: !isOrder ? (item as PurchaseInvoice).paidAmount || 0 : 0
    });
    setOpen(true);
  };
  ...
  return (
    <div style={{ padding: '20px' }}>
                  ...
                  <tbody>
                    {orders.map((order: PurchaseOrder) => (
                      <tr key={order.id} style={{ borderBottom: '1px solid #eee' }}>
                        ...
                        <td style={{ padding: '12px' }}>{order.supplierId}</td>
                  ...
                  <tbody>
                    {invoices.map((invoice: PurchaseInvoice) => (
                      <tr key={invoice.id} style={{ borderBottom: '1px solid #eee' }}>
                        ...
                        <td style={{ padding: '12px' }}>{invoice.purchaseOrderId}</td>
      ...
      <Dialog open={open} onClose={() => setOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>{editingItem ? 'Edit' : 'Add'} {tabValue === 0 ? 'Purchase Order' : 'Purchase Invoice'}</DialogTitle>
          ...
          <DialogContent>
            {tabValue === 0 ? (
              <>
                ...
                <TextField margin="dense" label="Supplier ID" type="number" value={formData.supplierId} onChange={(e) => setFormData({ ...formData, supplierId: parseInt(e.target.value) })} fullWidth required />
              ...
              <>
                ...
                <TextField margin="dense" label="Purchase Order ID" type="number" value={formData.purchaseOrderId} onChange={(e) => setFormData({ ...formData, purchaseOrderId: parseInt(e.target.value) })} fullWidth required />
